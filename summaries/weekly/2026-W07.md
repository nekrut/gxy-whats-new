...
This week, the Galaxy Project merged 79 pull requests across 14 repositories with contributions from 50 developers. The main Galaxy repository focused on migrating user interface components from BTable to GTable, converting tests to Playwright compatibility, and fixing bugs in AWS Batch monitoring, workflow downloads, and various UI issues. The team updated documentation and personnel information on the Galaxy Hub, added new bioinformatics tools and fixed deployment issues in the tools repository, and updated training materials with new links and dependency upgrades. Additional work included adding Hi-C workflows to the analytics repository and releasing version 0.22.0 with several bug fixes.
...
### iwc
This week, the Galaxy Project's "iwc" repository had one merged pull request that simplified and updated the influenza workflow. The changes focused on streamlining the existing workflow used for influenza analysis within the Galaxy platform.

### training-material
This week, the Galaxy Project training-material repository received updates to persistent uniform resource locators, added a link to an instructor Google form, and upgraded two dependencies (axios to version 1.13.5 and aws-actions/configure-aws-credentials to version 6). Contributors also updated cached commit data and resolved an issue regarding outdated FAQ content for dataset collections, while identifying a new issue about missing training examples in the BiaPy library.

### galaxy-hub
The Galaxy Project hub repository received updates to personnel information, including adding new team members Martin Demko and others while updating contact links and location details for existing members. The team also made several technical improvements including search page styling fixes, legacy asset serving in the Astro build system, dependency updates (axios library), and added new search functionality with Pan-Galactic and publication search tabs. Additionally, they synchronized training materials, fixed display issues with videos and images, and addressed a YAML parsing error in the people configuration file.

### galaxy-mcp
This week, the Galaxy Project's "galaxy-mcp" repository had one merged pull request that fixed a version mismatch between the __init__.py file and pyproject.toml file, updating both to version 1.3.0. This was a maintenance fix to ensure consistent version numbering across the project's configuration files.

### usegalaxy-tools
This week, the Galaxy Project's usegalaxy-tools repository received two updates. The kmindex tool was added to the assembly section, and IWC (Intergalactic Workflow Commission) tools were installed for cloud deployment scheduled for February 9, 2026.
...
### tools-iuc
This week, the Galaxy Project's tools-iuc repository merged 9 pull requests that included bug fixes and tool updates, such as adding CRAM file support to deepvariant, fixing deployment issues with haltools, and updating ToolDistillator to version 1.0.5. The team also addressed 14 closed issues ranging from technical problems like failing tests and autoupdate errors to feature requests for new tools like Fur for microGalaxy and samtools collate.

### galaxy
The Galaxy Project merged 28 pull requests this week, focusing heavily on migrating user interface components from BTable to GTable, converting end-to-end tests to Playwright compatibility, and applying multiple bug fixes across versions 25.0, 25.1, and 26.0. Key fixes included resolving crashes in AWS Batch monitoring, fixing tool retrieval by UUID, correcting workflow download issues with missing file sources, and addressing various UI problems like impersonation redirects and upload panel navigation. The team also updated dependencies including Pillow and cryptography, pinned setuptools versions to prevent compatibility issues, and enhanced database connection reliability.

### brc-analytics
The brc-analytics repository added Hi-C workflows and released version 0.22.0 this week. The team also fixed a bug that replaced duplicate version displays with an API status icon and resolved issues with release notes link visibility for unauthenticated users and release tagging problems.

### galaxy_codex
The Galaxy Project's "galaxy_codex" repository received three updates this week: an automatic resources update, fixes to lab population functionality and tools configuration files, and a dependency update for the Ruby setup component from version 1.287.0 to 1.288.0. A new issue was reported regarding silent failures in the weekly resource fetching and community filtering process for the Populate Lab feature.

### planemo
This week, one new issue was opened for the Galaxy Project's "planemo" repository requesting improvements to the version linter functionality. No other repository activity such as pull requests, commits, or releases occurred during this period based on the provided information.
...
This week, two new issues were reported for the Galaxy Project's "total-perspective-vortex" repository. Users identified a need to incorporate directory datatable entry sizes into the TPV system and reported a linting failure when using the resubmit with_more_mem_on_failure feature.
...
---
*Generated automatically on 2026-02-16 01:07 UTC*