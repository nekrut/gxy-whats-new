...
The Galaxy Project released version 26.0 release candidate 1 and began work on version 26.1, adding an AI-powered agent for tool recommendations and fixing workflow-related bugs. The team updated the Galaxy Hub website to resolve technical issues and added event pages, while the usegalaxy-tools repository received daily updates including new tool installations like pysradb_search and chainswap. Additional work included releasing brc-analytics version 0.21.0 with natural language search functionality and adding several new tools to the tools-iuc repository such as PIRATE and Mgnify genome search. With 20 repositories active, 84 pull requests merged, and 58 contributors participating, the week focused on platform improvements, tool additions, and bug fixes across the Galaxy ecosystem.
...
### galaxy-hub
The Galaxy Hub repository received updates to fix image display issues and other technical problems with the Astro framework, along with synchronization updates from the Galaxy Training Network materials. Several event pages were created or updated, including a new University Library Freiburg Galaxy Tools event, updates to the 2026 Metagenome Training Course, and additions of highlighted talks for GCC2026.

### usegalaxy-tools
The usegalaxy-tools repository received regular daily updates throughout the week, including cloud repository updates and installations of IWC (Intergalactic Workflow Commission) tools for usegalaxy.org. Several specific tools were updated or added, including egapx (updated to latest version), lexicmap wrapper (latest version), pysradb_search (newly installed), and chainswap (added to .org).

### tools-iuc
The Galaxy Project's tools-iuc repository added several new tools and updates this week, including the PIRATE tool, Mgnify genome search functionality, and version updates for VGP processcuration (1.0 to 1.1) and fastp (1.0.1 to 1.1.0). The team also fixed a form issue with the mitobim tool and resolved a problem with Medaka tools version 2.1.1+galaxy0 that was causing failures on Galaxy Australia and other instances.

### training-material
The Galaxy Project's training-material repository received updates to its organizational and contributor information, with new organizations and contributors added from the galaxy-hub. Several tutorial improvements were made, including updated API key examples, enhanced visualization steps, and updated persistent resource locators. The repository also added an external event for Love Data Week and included a new bio for BIONT organization.
...
### galaxy
This week, the Galaxy Project released version 26.0 release candidate 1 and began development on version 26.1. Key changes included adding an AI-powered agent for tool recommendations, fixing several workflow-related bugs including issues with the "Discover Tools" button and auto-layout functionality, and resolving problems with user sharing features and storage management for anonymous users.

### bioblend
This week, the Galaxy Project's bioblend repository had one merged pull request that added a new method for extracting help text from tools. This enhancement provides users with programmatic access to tool documentation through the bioblend API.

### brc-analytics
The Galaxy Project's brc-analytics repository released version 0.21.0 this week, which added LLM-powered natural language search functionality and configured separate backend URLs for development and production environments. Other changes included adding a white papers section to the homepage, removing a survey card, improving the workflows quality control report, and updating the findable-ui dependency to the latest version.

### galaxy-k8s-boot
This week, the Galaxy Project's galaxy-k8s-boot repository had one merged pull request that added masthead images to the project. The change appears to involve adding visual header elements, likely for the user interface of the Kubernetes-based Galaxy deployment system.

### galaxy_codex
This week the Galaxy Project updated several components in the galaxy_codex repository, including improvements to the imaging lab, interactive tables, and an automatic resources update. The team also upgraded the Ruby setup dependency from version 1.284.0 to 1.286.0 and opened a new issue to discuss promoting peer-reviewed content in Labs, specifically related to SPOC.

### galaxy-mcp
The Galaxy Project updated their "galaxy-mcp" repository this week with a single merged pull request that incremented the schema version and bumped the package version to 1.3.0. This appears to be a routine version update for the project.

### galaxy-visualizations
This week, the Galaxy Project's visualizations repository received four updates focused on bug fixes and feature additions. The changes included restoring a default value, fixing plot height issues in the LocusZoom visualization tool, adding a materializer component to the Polaris visualization, and updating the chromosome input type for LocusZoom along with a version upgrade.

### planemo
This week, one new issue titled "Planemo improvements" was created in the Galaxy Project's planemo repository. No other specific changes or updates were recorded for the repository during this period.

### iwc
This week, one new issue was opened in the Galaxy Project's "iwc" repository regarding testing release 26.0. No other repository activity or changes were reported during this period.

### gxy.io
This week, the Galaxy Project's gxy.io repository had one merged pull request that added a JXTX URL shortener for BoG (Board of Governors). This appears to be a small addition to the URL shortening functionality of the gxy.io service.

### galaxy-release-util
This week, one new issue was reported in the Galaxy Project's galaxy-release-util repository. The issue identifies a problem where the utility appears to make incorrect assumptions about previous or next release versions. No other development activity occurred during this period.
...
---
*Generated automatically on 2026-02-01 19:34 UTC*