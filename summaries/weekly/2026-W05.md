...
The Galaxy Project released version 26.0 release candidate 1 and began development on version 26.1, with 84 pull requests merged across 20 repositories by 58 contributors. Key developments included adding an AI-powered tool recommendation feature to the main Galaxy platform and implementing LLM-powered natural language search functionality in the brc-analytics repository. The team fixed various bugs affecting workflow sharing, user interface components, and tool functionality, while adding new tools like PIRATE and Mgnify genome search to the tools collection. Documentation and event pages were updated on Galaxy Hub, and routine tool synchronizations continued across Galaxy instances.
...
| [galaxy-hub](https://github.com/galaxyproject/galaxy-hub) | 19 | 12 | 3 | 1 |
...
### tools-iuc
This week, the Galaxy Project's tools-iuc repository added the PIRATE tool and Mgnify genome search functionality, while updating the VGP process curation tool to version 1.1 and fastp to version 1.1.0. The repository also resolved a bug affecting Medaka tools version 2.1.1+galaxy0 that was causing issues on Galaxy Australia and other Galaxy instances.

### usegalaxy-tools
This week, the usegalaxy-tools repository received multiple routine updates including daily repository synchronizations and installations of tools from the Intergalactic Workflow Commission (IWC). Several specific tools were added or updated, including egapx (updated to its latest version), lexicmap wrapper (latest version), pysradb_search, and chainswap for the main Galaxy instance.

### galaxy
The Galaxy Project released version 26.0 release candidate 1 and began development on version 26.1, while merging bug fixes from the 25.1 branch. Key changes include adding an AI-powered tool recommendation feature, fixing issues with workflow sharing and user interface components, improving HTCondor job runner functionality, and resolving problems with data manager file selection and anonymous user storage management.

### brc-analytics
The brc-analytics repository added LLM-powered natural language search functionality and a white papers section to the homepage, while removing the survey card. The team also improved the release workflow to handle different branch versions, enhanced the workflows quality control report, and updated the findable-ui dependency to the latest version.

### galaxy-hub
The Galaxy Hub repository received updates to fix technical issues with the Astro framework, including missing images in subdirectories and URL routing problems. Several new event pages were created or updated, including events for Galaxy Tools at University Library Freiburg, a NUBRI kickoff, and the 2026 Metagenome Training Course, while training material files were synchronized from the Galaxy Training Network.

### training-material
The Galaxy Project training-material repository received updates to expand its organizational coverage and improve tutorial content. Contributors added new organizations and funding bodies from galaxy-hub, updated grant information, and enhanced tutorials with pre-visualization features and updated API key examples. The repository also received routine maintenance including cached commit data updates and persistent URL updates.
...
### bioblend
This week, the Galaxy Project's bioblend repository had one merged pull request that added a new method for extracting help text from tools. This enhancement allows users to programmatically access tool documentation through the bioblend API.
...
### galaxy-mcp
The Galaxy Project updated the galaxy-mcp repository by merging one pull request that incremented the schema version and bumped the package version to 1.3.0. This appears to be a routine version update to maintain compatibility or introduce minor changes to the project's schema structure.

### galaxy_codex
This week the Galaxy Project's "galaxy_codex" repository received updates to its imaging lab content and interactive tables, along with an automatic resources update and a Ruby dependency bump from version 1.284.0 to 1.286.0. A new issue was opened to discuss promoting peer-reviewed content in Labs, specifically regarding SPOC (Single Point of Contact) implementation.

### iwc
This week, one new issue was opened in the Galaxy Project's "iwc" repository requesting testing for release 26.0. No other activity details were provided for the repository during this period.

### gxy.io
This week, the Galaxy Project's gxy.io repository had one merged pull request that added a JXTX shortener for BoG (presumably "Bits of Galaxy" or a similar Galaxy-related resource). The change appears to expand the URL shortening capabilities of the gxy.io service to include JXTX-related links.

### planemo
This week, one new issue was opened in the Galaxy Project's planemo repository titled "Planemo improvements," though no specific details about the proposed improvements were provided in the issue summary. No other development activity, such as code commits or pull requests, was reported for the repository during this period.

### galaxy-release-util
This week, one new issue was reported for the Galaxy Project's "galaxy-release-util" repository regarding a utility that may be incorrectly assuming invalid previous or next release versions. No code changes, pull requests, or other development activity occurred during this period. The repository's activity was limited to this single bug report.
...
---
*Generated automatically on 2026-02-01 19:45 UTC*