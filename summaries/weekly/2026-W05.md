...
The Galaxy Project released version 26.0.rc1 this week and began development on version 26.1, with 84 pull requests merged across 20 repositories by 58 contributors. Key developments included adding an AI-powered tool recommendation system to the main Galaxy platform and implementing LLM-powered natural language search functionality in the analytics repository. The team focused on bug fixes for workflow sharing, user interface issues, and storage management problems, while also adding and updating various bioinformatics tools including PIRATE, Mgnify genome search, and updated versions of existing tools. Documentation and training materials were updated on Galaxy Hub, including new event pages and training course information.
...
### usegalaxy-tools
This week, the usegalaxy-tools repository received 12 merged pull requests focused on routine updates and tool installations. The updates included daily repository synchronizations, installations of IWC (Intergalactic Workflow Commission) tools for usegalaxy.org and cloud instances, and additions of specific tools including the latest version of egapx, an updated lexicmap wrapper, pysradb_search, and chainswap.

### galaxy-hub
The Galaxy Hub repository received updates to fix missing images and other technical issues in the Astro web framework, along with synchronization of training materials and metadata from the Galaxy Training Network. Several event pages were created or updated, including a new Galaxy Tools event at University Library Freiburg, updates to the 2026 Metagenome Training Course, and the addition of highlighted talks for GCC2026.

### training-material
The Galaxy Project training-material repository received updates to add external events, organizations, and contributors from the galaxy-hub, including Love Data Week and new funding organizations like AusBioCommons and UniMelb. Contributors also enhanced tutorials by adding pre-visualization steps, updating API key examples, and fixing persistent resource locators. Additionally, contributor profiles were updated with a new bio for BIONT and various tweaks for hub integration.

### tools-iuc
This week, the Galaxy Project's tools-iuc repository added the PIRATE tool and Mgnify genome search functionality, while updating two existing tools: vgp_processcuration to version 1.1 and fastp to version 1.1.0. The team also fixed a form issue with the mitobim tool and resolved a problem with Medaka tools version 2.1.1+galaxy0 that was causing failures on Galaxy Australia and other instances.

### brc-analytics
This week the Galaxy Project's brc-analytics repository added LLM-powered natural language search functionality and configured separate backend URLs for development and production environments. The team also updated the homepage by removing the survey card and adding a white papers section, while improving the release workflow to handle different branch versions and enhancing the workflows quality control report.

### usegalaxy-playbook
This week, users submitted two tool installation requests for the Galaxy Project's usegalaxy-playbook repository. One request was for the gwtc_analysis tool for astronomy research, and another was to install the bcftools_plugin_split_vep tool on the ORG and AU Galaxy servers, as it is currently only available on the EU server.

### bioblend
This week, the Galaxy Project's bioblend repository merged one pull request that added a new method for extracting help text from tools. This enhancement allows users to programmatically access tool documentation and usage information through the bioblend API.

### galaxy_codex
This week, the Galaxy Project's "galaxy_codex" repository received updates to its imaging lab content and interactive tables, along with an automatic resources update and a dependency bump for ruby/setup-ruby from version 1.284.0 to 1.286.0. A new issue was opened to discuss promoting peer-reviewed content in Labs, specifically focusing on SPOC (Self-Paced Online Course) materials.

### galaxy
Galaxy developers released version 26.0.rc1 and began development on version 26.1, while merging maintenance fixes from version 25.1 into the main development branch. The week's changes included bug fixes for workflow sharing, user interface issues in sharing dialogs, and storage management problems for anonymous users, along with the addition of an AI-powered tool recommendation system. New issues were reported around database migration errors, workflow editor problems, and requests for interface improvements including replacing table components and redesigning the upload interface.

### galaxy-k8s-boot
This week, the galaxy-k8s-boot repository had one merged pull request that added masthead images to the project. The change appears to involve updating the visual header elements for the Galaxy Kubernetes deployment tool.

### galaxy-mcp
The Galaxy Project's "galaxy-mcp" repository released version 1.3.0 this week. The update included changes to the schema version along with the standard package version bump to reflect the new release.

### galaxy-visualizations
This week, the Galaxy Project's visualization repository received four updates focused on bug fixes and feature additions. The changes included restoring a default value, fixing plot height issues in the LocusZoom visualization tool, adding a materializer component to the Polaris visualization, and updating the chromosome input type in LocusZoom along with a version upgrade. These modifications address display problems and enhance the functionality of existing visualization tools within the Galaxy platform.

### iwc
This week, the Galaxy Project's "iwc" repository had one new issue opened regarding testing release 26.0. No other repository activity was reported for this period.

### planemo
This week, the Galaxy Project's "planemo" repository had minimal activity with only one new issue opened titled "Planemo improvements." No other specific changes, pull requests, or commits were reported for this period.

### gxy.io
This week, the Galaxy Project's gxy.io repository had one merged pull request that added a JXTX shortener for BoG (Board of Governors). This appears to be a URL shortening feature related to the JXTX Foundation and Galaxy's governance structure.

### galaxy-release-util
One new issue was reported in the galaxy-release-util repository this week regarding the utility making incorrect assumptions about previous or next release versions. No other development activity, such as commits, pull requests, or releases, was recorded during this period.
...
---
*Generated automatically on 2026-02-01 18:51 UTC*