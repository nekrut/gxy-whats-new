...
The Galaxy Project released version 26.0 release candidate 1 and began development on version 26.1, while continuing maintenance work on the 25.1 branch. Key additions included an AI-powered tool recommendation feature and LLM-based natural language search functionality in the analytics platform. The team resolved workflow-related bugs affecting sharing, storage management, and authentication systems, while also fixing technical issues with the Galaxy Hub website framework. Regular tool updates continued across Galaxy instances, with new tools like PIRATE and Mgnify genome search added to the collection.
...
### tools-iuc
This week, the Galaxy Project's tools-iuc repository added new tools including PIRATE and Mgnify genome search, while updating existing tools like fastp (to version 1.1.0) and vgp_processcuration (to version 1.1). The team also fixed a broken Medaka tools issue that was affecting Galaxy Australia and other Galaxy instances, and addressed a form problem with the mitobim tool.

### usegalaxy-tools
This week, the Galaxy Project's usegalaxy-tools repository received regular automated updates to install tools from the Intergalactic Workflow Commission (IWC) and synchronize repositories across different Galaxy instances. Several specific tools were added or updated, including egapx (updated to the latest version), lexicmap wrapper (latest version), pysradb_search, and chainswap for the main Galaxy.org instance.

### galaxy-hub
The Galaxy Hub repository received updates to fix technical issues with the Astro framework, including problems with missing images in subdirectories and URL routing inconsistencies. Several event pages were created or updated, including a Galaxy Tools event at University Library Freiburg, a 2026 Metagenome Training Course, and a NUBRI kickoff event, while content was synchronized from the Galaxy Training Network materials.
...
### training-material
The Galaxy Project training-material repository received updates to add new organizations and contributors from the galaxy-hub, including funding organizations like AusBioCommons and UniMelb, plus a new bio for BIONT. Several tutorials were enhanced with updated API key examples, pre-visualization features, and improved steps, while technical maintenance included updating cached commit data and persistent resource locators.

### galaxy
Galaxy developers released version 26.0 release candidate 1 and began work on version 26.1, while also merging fixes from the 25.1 maintenance branch. The week's changes included adding an AI-powered tool recommendation feature, fixing workflow-related issues like duplicated sharing entries and storage management problems for anonymous users, and resolving various bugs in data managers, job runners, and user authentication systems.

### brc-analytics
The Galaxy Project's brc-analytics repository released version 0.21.0 this week, adding LLM-powered natural language search functionality and configuring separate backend URLs for development and production environments. The team also updated the homepage by removing the survey card and adding a new white papers section, while completing several workflow improvements including enhanced QC reporting and differential expression analysis features.

### galaxy-k8s-boot
This week, the Galaxy Project's "galaxy-k8s-boot" repository had one merged pull request that added masthead images to the project. The change appears to be related to user interface elements, likely adding visual branding or navigation graphics to the Galaxy Kubernetes deployment tool.

### bioblend
This week, the Galaxy Project's bioblend repository merged one pull request that added a new method for extracting help text from tools. This enhancement provides developers with programmatic access to tool documentation and usage information through the bioblend API.

### galaxy-visualizations
This week, the Galaxy Project's visualization repository received four updates focused on bug fixes and feature additions. The changes included restoring a default value, fixing plot height issues in the LocusZoom visualization tool, adding a materializer component to the Polaris visualization, and updating the chromosome input type for LocusZoom while upgrading its version. These modifications address display problems and enhance the functionality of existing visualization tools within the Galaxy platform.

### galaxy_codex
This week, the Galaxy Project's "galaxy_codex" repository received updates to its imaging lab content and interactive tables, along with an automatic resources update. The development dependencies were also updated with a Ruby setup version bump from 1.284.0 to 1.286.0, and a new issue was opened to discuss promoting peer-reviewed content within the Labs section.
...
### iwc
This week, the Galaxy Project's "iwc" repository had one new issue opened requesting testing for release 26.0. No other significant activity or changes were reported for the repository during this period.

### gxy.io
This week, the Galaxy Project's "gxy.io" repository had one merged pull request that added a URL shortener functionality specifically for "BoG" (likely Board of Governors or similar Galaxy Project governance content). The change expands the repository's URL shortening capabilities to include this new "jxtx" shortener service.

### galaxy-release-util
This week, one new issue was reported for the galaxy-release-util repository regarding a utility that appears to incorrectly assume invalid previous or next release versions. No other development activity, such as code commits or pull requests, was recorded during this period.

### planemo
One new issue was opened in the Galaxy Project's planemo repository this week titled "Planemo improvements," though no specific details about the proposed improvements were provided in the issue title. No other repository activity was reported for this period.
...
---
*Generated automatically on 2026-02-01 19:08 UTC*