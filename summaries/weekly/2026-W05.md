...
The Galaxy Project community merged 85 pull requests across 20 repositories with contributions from 58 developers this week. The main Galaxy repository released version 26.0.rc1 and added an AI-powered tool recommendation system, while fixing workflow-related issues and user interface problems including duplicated sharing dialog entries. Several repositories received routine maintenance updates, with usegalaxy-tools synchronizing tool installations and tools-iuc adding new tools like PIRATE and updating existing ones such as fastp and VGP processcuration. The project also updated its website infrastructure, published training materials, and released brc-analytics version 0.21.0 with natural language search capabilities.
...
### usegalaxy-tools
This week, the usegalaxy-tools repository received 12 merged pull requests focused on routine maintenance and tool updates. The changes included daily repository updates and installations of IWC (Intergalactic Workflow Commission) tools for usegalaxy.org, along with updates to specific tools including egapx, lexicmap wrapper, pysradb_search, and chainswap. Most activity involved automated updates to keep the cloud and main repositories synchronized with the latest tool versions.

### galaxy-hub
The Galaxy Project's hub repository received updates to its website infrastructure, including fixes for missing images in subdirectories and multiple smaller technical issues. Several events were added or updated, including a Galaxy Tools workshop at University Library Freiburg, a 2026 Metagenome Training Course, and highlighted talks for GCC2026. The team also synchronized training materials, added schema files, and published the weekly newsletter for 2026-W05.

### training-material
The Galaxy Project training material repository received updates to organizational information, including new organizations and contributors from the galaxy hub, along with grants and funding organization data for AusBioCommons and UniMelb. Several tutorials were enhanced with updated API key examples, pre-visualization features, and improved steps, while administrative updates included adding a Love Data Week external event and refreshing cached commit data and resource locators.

### brc-analytics
The Galaxy Project's brc-analytics repository released version 0.21.0 this week, adding LLM-powered natural language search functionality and a white papers section to the homepage while removing the survey card component. The team also improved the release workflow to handle different branch versions, enhanced the workflows quality control report, and updated the findable-ui dependency to the latest version.

### tools-iuc
This week, the Galaxy Project's tools-iuc repository added the PIRATE tool and Mgnify genome search functionality, while updating the VGP processcuration tool to version 1.1 and fastp to version 1.1.0. The team also fixed a form issue with the mitobim tool and resolved a problem with Medaka tools version 2.1.1+galaxy0 that was causing failures on Galaxy Australia and other Galaxy instances.

### galaxy
The Galaxy Project repository released version 26.0.rc1 and started development on version 26.1.dev, while also merging maintenance updates from the 25.1 branch. Key changes include adding an AI-powered tool recommendation system, fixing workflow-related issues such as storing origin for landing requests and removing the non-functional "Discover Tools" button from the workflow editor, and resolving several user interface problems including duplicated entries in sharing dialogs and storage management checks for anonymous users.
...
### usegalaxy-playbook
This week, the Galaxy Project's usegalaxy-playbook repository received two tool installation requests. Users requested the addition of gwtc_analysis for astronomy work and bcftools_plugin_split_vep to be installed on the ORG and AU Galaxy servers, as this tool is currently only available on the EU server.

### galaxy_codex
This week's activity for the galaxy_codex repository included updates to the imaging lab content and interactive tables, plus an automatic resources update. The Ruby setup dependency was bumped from version 1.284.0 to 1.286.0, and a new issue was opened to discuss promoting peer-reviewed content in Labs, specifically focusing on SPOC (Single Point of Contact) implementation.

### galaxy-k8s-boot
This week, the Galaxy Project's "galaxy-k8s-boot" repository had one merged pull request that added masthead images to the project. This appears to be a visual enhancement to the user interface components of the Kubernetes-based Galaxy deployment tool.

### galaxy-mcp
The Galaxy Project updated the galaxy-mcp repository this week by releasing version 1.3.0. The update included changes to the schema version and bumped the overall package version number.
...
### iwc
This week, the Galaxy Project's "iwc" (Intergalactic Workflow Commission) repository had one new issue opened regarding testing release 26.0. No other significant activity or changes were reported for the repository during this period.

### planemo
This week, the Galaxy Project's "planemo" repository had one new issue opened titled "Planemo improvements." No other specific changes or activities were reported in the provided information.

### gxy.io
This week, the Galaxy Project's "gxy.io" repository had one merged pull request that added a JXTX shortener for BoG (likely referring to "Birds of a Feather" sessions or meetings). This appears to be a URL shortening feature added to the Galaxy Project's web infrastructure.

### galaxy-release-util
This week, one new issue was reported for the Galaxy Project's galaxy-release-util repository. The issue concerns a potential problem where the utility tool may be making incorrect assumptions about previous or next release version numbers.
...
---
*Generated automatically on 2026-02-02 01:26 UTC*