...
The Galaxy Project released version 26.0.rc1 this week and began development on version 26.1.dev, with 84 pull requests merged across 20 repositories by 58 contributors. Development focused on workflow improvements, including fixes for landing requests and tool panel issues, plus the addition of AI-powered tool recommendations. The team addressed authentication bugs, job management problems with HTCondor, and various user interface issues. Repository updates included new tool installations, LLM-powered search functionality for analytics, and documentation updates for upcoming training events.
...
### usegalaxy-tools
This week, the usegalaxy-tools repository received 12 merged pull requests focused on routine updates and new tool installations. The activity included daily repository updates and installations of IWC (Intergalactic Workflow Commission) tools for usegalaxy.org, plus updates to specific tools like egapx and lexicmap wrapper, and the addition of new tools including pysradb_search and chainswap.

### tools-iuc
This week, the Galaxy Project's tools-iuc repository added the PIRATE tool and Mgnify genome search functionality, while updating the VGP process curation tool to version 1.1 and fastp to version 1.1.0. The team also fixed a form issue with the mitobim tool and resolved a problem with Medaka tools that were broken on Galaxy Australia and other Galaxy instances.

### galaxy-hub
The Galaxy Project hub repository received updates to fix image handling and other issues in the Astro framework, along with synchronization of content from training materials. Several event pages were created or updated, including a Galaxy Tools event at University Library Freiburg, a 2026 Metagenome Training Course, and a NUBRI kickoff event with added supporter information. Two new issues were opened regarding URL routing problems and accessibility concerns, while one URL routing issue was resolved.

### brc-analytics
This week, the Galaxy Project's brc-analytics repository added LLM-powered natural language search functionality and configured separate backend URLs for development and production environments. The team also updated the homepage by adding a white papers section and temporarily removing the survey card, while making improvements to the release workflow and workflows quality control reporting.

### training-material
The Galaxy Project training-material repository received updates to event listings, organizational information, and contributor data this week. New organizations from galaxy-hub were added along with grant information updates, and a tutorial was enhanced with pre-visualization features and updated steps. Additionally, the repository updated cached commit data and persistent resource locators, while addressing issues related to community lab training improvements and funding organization additions.

### galaxy
The Galaxy Project released version 26.0.rc1 and started development on version 26.1.dev this week. Key fixes included resolving workflow-related issues such as storing origin for workflow landing requests, removing the "Discover Tools" button from the workflow tool panel, and fixing duplicated entries in the user sharing component. The team also added an AI-powered tool recommendation feature, improved type annotations throughout the codebase, and addressed various authentication and job management bugs including HTCondor runner issues and OIDC username validation problems.

### usegalaxy-playbook
This week, the Galaxy Project's usegalaxy-playbook repository received two new tool installation requests. Users requested the addition of gwtc_analysis for astronomy workflows and bcftools_plugin_split_vep for the ORG and AU Galaxy instances, as this tool is currently only available on the EU instance.

### galaxy-k8s-boot
This week, the Galaxy Project's "galaxy-k8s-boot" repository had one merged pull request that added masthead images to the project. This appears to be a user interface update that incorporates visual branding elements into the Kubernetes-based Galaxy deployment tool.

### galaxy-visualizations
This week, the Galaxy Project's visualization tools repository received four updates focused on bug fixes and feature additions. The changes included restoring a default configuration value, fixing display height issues in the LocusZoom visualization tool, and adding a materializer component to the Polaris visualization. Additionally, the LocusZoom tool was upgraded to a newer version with modifications to how chromosome data is handled.

### bioblend
This week, the Galaxy Project's bioblend repository had one merged pull request that added a new method for extracting help text from tools. This enhancement allows users to programmatically access tool documentation and usage information through the bioblend API.

### galaxy-mcp
The Galaxy Project released version 1.3.0 of the galaxy-mcp repository this week. The update included changes to the schema version and bumped the overall package version to reflect these modifications.

### galaxy_codex
This week the Galaxy Project's "galaxy_codex" repository received updates to its imaging lab content and interactive tables, along with an automatic resources update and a dependency bump for ruby/setup-ruby from version 1.284.0 to 1.286.0. A new issue was opened discussing how to promote peer-reviewed content in Labs, specifically related to SPOC (Self-Paced Online Courses).

### iwc
This week, the Galaxy Project's "iwc" repository had one new issue opened regarding testing release 26.0. No other notable activity or changes were reported for the repository during this period.

### planemo
This week, one new issue titled "Planemo improvements" was opened in the Galaxy Project's planemo repository. No other specific changes or updates were documented in the provided activity summary.
...
### galaxy-release-util
This week, one new issue was reported in the galaxy-release-util repository regarding the utility incorrectly assuming invalid previous or next release versions. No other development activity such as code commits, pull requests, or issue resolutions occurred during this period.
...
---
*Generated automatically on 2026-02-01 19:20 UTC*