...
The Galaxy Project released version 26.0 release candidate 1 and began work on version 26.1, which includes a new AI-based tool recommendation system. Development work addressed workflow bugs, data manager issues, and HTCondor job monitoring problems across the main codebase. The community hub received updates for upcoming events including GCC2026 and various training courses, while technical fixes resolved framework issues and broken links. Tool repositories saw regular maintenance with daily installations, addition of new tools like PIRATE and Mgnify genome search, and updates to existing tools, while the analytics platform gained LLM-powered natural language search capabilities in version 0.21.0.
...
### tools-iuc
The Galaxy Project's tools-iuc repository added two new tools this week (PIRATE and Mgnify genome search) and updated two existing tools (vgp_processcuration to version 1.1 and fastp to version 1.1.0). The team also fixed a form issue with the mitobim tool and resolved a problem with Medaka tools that were broken on Galaxy Australia and other Galaxy instances.

### galaxy-hub
This week, the Galaxy Project's galaxy-hub repository focused on event management and technical fixes. Several new event pages were created or updated, including events for Galaxy Tools at University Library Freiburg, the NUBRI kickoff, and the 2026 Metagenome Training Course, while highlighted talks were added for GCC2026. The team also addressed technical issues with the Astro framework, fixed missing images and schema files, and resolved URL routing problems in the Hall of Fame section.

### usegalaxy-tools
The Galaxy Project's usegalaxy-tools repository received regular maintenance updates throughout the week, with daily installations of Intergalactic Workflow Commission (IWC) tools and repository updates occurring from January 26-30 and February 1. Several specific tools were added or updated, including egapx (updated to the latest version), lexicmap wrapper (latest version), pysradb_search (new installation), and chainswap (added to the main .org instance).

### training-material
The Galaxy Project training-material repository received updates to organizational information, including new organizations and contributors from galaxy-hub, grants data, and a bio for BIONT. Tutorial content was enhanced with pre-visualization features and updated steps, while technical maintenance included updating cached commit data and persistent resource locators. An external event for Love Data Week was also added to the repository.

### brc-analytics
The Galaxy Project's "brc-analytics" repository released version 0.21.0 this week, adding LLM-powered natural language search functionality and configuring separate backend URLs for development and production environments. The team also updated the homepage by removing the survey card and adding a white papers section, while making improvements to workflow quality control reports and release processes.
...
### galaxy-k8s-boot
This week, the Galaxy Project's "galaxy-k8s-boot" repository had one merged pull request that added masthead images to the project. The change appears to involve visual elements for the user interface header area of the Galaxy platform's Kubernetes deployment configuration.

### galaxy_codex
This week, the Galaxy Project's galaxy_codex repository received four merged updates: an imaging lab update, automatic resources update, a Ruby setup dependency bump from version 1.284.0 to 1.286.0, and interactive tables improvements. One new issue was opened regarding the promotion of peer-reviewed content in Labs, specifically related to SPOC (Self-Paced Online Courses).

### bioblend
This week, the Galaxy Project's bioblend repository merged one pull request that added a new method for extracting help text from tools. This enhancement allows users to programmatically access tool documentation through the bioblend API.

### galaxy-mcp
The Galaxy Project released version 1.3.0 of the galaxy-mcp repository this week. The update included changes to the schema version along with the standard package version increment to 1.3.0.

### galaxy
The Galaxy Project released version 26.0 release candidate 1 this week and began development on version 26.1. Key changes included adding an AI-based tool recommendation system, fixing workflow-related bugs such as duplicated user sharing entries and landing request storage issues, and resolving problems with data manager file selection logic and HTCondor job monitoring.

### iwc
This week, one new issue was opened in the Galaxy Project's "iwc" repository regarding testing release 26.0. No other activity details were provided for the repository during this period.

### planemo
This week, the Galaxy Project's "planemo" repository had one new issue opened titled "Planemo improvements." No other specific development activity or changes were documented in the provided information.

### gxy.io
This week, the Galaxy Project's "gxy.io" repository had one merged pull request that added a JXTX shortener for BoG (Board of Governors). The change appears to be a URL shortening service addition related to the JXTX Foundation and Galaxy's governance structure.

### galaxy-release-util
This week, one new issue was reported in the galaxy-release-util repository regarding the utility incorrectly assuming invalid previous or next release versions. No code changes, pull requests, or other development activity occurred during this period.
...
---
*Generated automatically on 2026-02-01 19:36 UTC*