...
This week, 49 contributors merged 87 pull requests across 21 repositories. The Galaxy server received bug fixes in the workflow editor, markdown rendering, and data handling, along with storage quota enforcement for Celery-based fetch jobs and the switch to Tool Shed 2 as the default. The Galaxy Hub website completed its migration from Gridsome to Astro, removing legacy code and fixing rendering issues introduced during the transition, while also adding new content about Galaxy for particle accelerators and Jetstream2 genome assemblies. The training materials, community workflows, and IUC tools saw incremental updates including tutorial clarifications for Galaxy 25.1+, tool version bumps, and bug fixes.
...
| [galaxy_codex](https://github.com/galaxyproject/galaxy_codex) | 3 | 2 | 0 | 0 |
...
### galaxy
This week's work on the Galaxy repository included multiple bug fixes across the workflow editor, markdown rendering, and data handling—such as fixing infinite request loops in cached stores, correcting collection step rendering in reports and markdown, repairing the "Create New" workflow editor state reset, and fixing dbkey filtering when the dbkey is unset. Storage quota enforcement was added for Celery-based data fetch jobs, and the Tool Shed 2 was made the default by dropping legacy Shed tests from CI. Other changes improved filesystem path construction by replacing f-strings with `os.path.join()`/`pathlib.Path`, refined the Type and Reference selectors in the Upload Activity, and added a shared `format_xml` utility to galaxy-tool-util.

### training-material
This week, the Galaxy Training Material repository received updates to several tutorials, including clarifications about nested collections in Galaxy 25.1+, updates to the RStudio in Galaxy tutorial, and a new tip added to the Mapping tutorial about setting dbkey for samtools stats. Infrastructure changes included adding a check to avoid rendering empty tool install files, fixing redirects for Hall of Fame pages, and updating persistent URLs and cached commit data. A news item was added for a misconceptions paper, along with a recommendation on citing public workflows, and several issues related to broken tool links and tutorial usability were closed.

### galaxy-hub
This week's work on galaxy-hub focused heavily on completing the migration from the old Gridsome-based site to the new Astro-based site: the Gridsome code was removed from the master branch, CI was switched to Astro-only, and preparations were made to rename the default branch from "master" to "main." Several bugs introduced by the migration were fixed, including Vue server-side rendering hydration mismatches caused by date handling and apostrophes in titles, broken community sub-page links, and duplicate content ID warnings. Content updates included new news posts about Galaxy for particle accelerators and Jetstream2 genome assemblies, a microGalaxy roadmap page, updated meeting details for the SPOC and Tool Development SIGs, and NFDI4BioImage annotations on news and events pages.

### galaxy-job-config-init
A pull request was merged that adds a `--local-workers` option, allowing users to specify the number of workers for the LocalJobRunner. This was the only change to the repository this week.

### tools-iuc
This week, the tools-iuc repository merged five pull requests: a fix for Cutadapt's `--minimum-length` parameter default, a bug fix for HyPhy's annotate invert flag, an update to PretextMap with a new `--highres` option, a version correction for ToolDistillator's Galaxy packaging, and an update of mosdepth from version 0.3.12 to 0.3.13. One new issue was opened regarding Samtools Stats not showing reference genomes, and two issues were closed, including that same Samtools Stats issue and a Kraken2 bug involving incorrect datatype assignment for zipped input datasets.
...
The repository updated the `minimatch` dependency from version 3.1.2 to 3.1.4 in the client directory, addressing a known vulnerability or compatibility issue. A separate change improved how the Python environment is set up for the language server.

### pulsar
A single pull request was merged this week, removing the project's dependency on three Python packages: `distutils`, `pkg_resources`, and `stopit`. This change cleans up outdated or deprecated dependencies, as `distutils` and `pkg_resources` are legacy Python packaging tools that have been superseded by modern alternatives.

### iwc
Two dependency packages (lodash-es/mermaid and minimatch) were updated in the project website. On the workflow side, the genes catalogue metagenomics workflow was updated, and the RNA-seq differential expression workflow was bumped from version 0.7 to 0.8. A test release issue (26.0) was also closed.

### usegalaxy-tools
A pull request was merged to add imaging tools from usegalaxy.eu. Two new requests were opened: one to increase computational resources for the optitype tool and another to install the qiime2 import-fastq tool on UseGalaxy.org. A previously reported bug involving a permissions error in Funannotate predict was closed.
...
### galaxy-skills
Two pull requests were merged this week. One added a new "tool-selection-diagram" sub-skill to the tool-dev skill area. The other fixed and updated the hub-news-posts skill based on lessons learned from actually creating a post.
...
### gx-it-proxy
A single pull request was merged this week, updating the "minimatch" dependency to a newer version. This is a routine dependency bump, likely to incorporate bug fixes or security patches from the upstream library.
...
A new issue was reported this week identifying a bug where linting fails for `tool.tool_type`. No other activity (pull requests, merges, or releases) was noted in the repository during this period.

### gravity
Two pull requests were merged to the Galaxy Project's "gravity" repository this week. One added a dispatch option based on the gunicorn version, and the other fixed an issue with version checking.

### idc
This week, a new issue was opened in the IDC repository to discuss data documentation. No other activity (such as pull requests, merges, or code changes) was recorded for the repository during this period.
...
---
*Generated automatically on 2026-03-01 09:20 UTC*